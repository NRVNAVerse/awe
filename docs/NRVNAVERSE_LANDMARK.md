...
Read-only audit 2B.4B.1 (no repo change), then branch `feat/m0-http-cache-delivery` (2B.4B.2, from `nrvna/integration`). Performance step delivered as **HTTP delivery policy only** — no new travel architecture, no application cache, no prefetch, no retained chunks, no gate change. Full description: [`NRVNAVERSE_SPATIAL_DATA_PIPELINE.md`](./NRVNAVERSE_SPATIAL_DATA_PIPELINE.md) §15 and [`NRVNAVERSE_SPATIAL_RUNTIME.md`](./NRVNAVERSE_SPATIAL_RUNTIME.md) §15.
...
| Decision B | HTTP delivery follow-up only. **Rejected for M0:** application raw/validated payload cache (the browser HTTP cache already holds the bytes; a second copy adds memory, invalidation and a stale-data surface), predictive/neighbour prefetch (fetches chunks the visitor may never enter; a gated neighbour would violate gate-before-fetch, D-006), instantiated-chunk retention, Service Worker / CacheStorage / IndexedDB / localStorage, hashed file names (needs an output-lifecycle design). |
| Content versioning | `globalSceneUrl` and every `chunks[key].dataUrl` in the generated index now end in `?v=<token>`, token = first 32 lowercase hex chars (128 bits) of SHA-256 over the exact serialized artifact text (`node:crypto`, no dependency). Same content → same token; any byte change → new token → new URL. File names, chunk keys, destination ids, gates and `schemaVersion` (1) unchanged; no new index field. Generation order: artifact texts → tokens → index (no circularity). Second `spatial:generate` writes nothing; the index is the only regenerated file (2 827 bytes). |
| Version root | `/data/spatial/spatial-index.json` keeps its fixed URL and is served `public, max-age=0, must-revalidate` (explicit rule); it points at the immutable versioned artifacts. |
| Cache rules (`next.config.ts` `headers()`) | `/data/spatial/global-scene.json` and `/data/spatial/chunks/:chunk([a-z0-9-]+).json` → `public, max-age=31536000, immutable` **only** with `has: [{ type: "query", key: "v", value: "[0-9a-f]{32}" }]` (anchored by Next; confirmed against the installed Next 16.1.6 matcher and the built `routes-manifest.json`). |
| Unversioned safety | The same paths without `?v=`, with a malformed/short/uppercase token or another parameter name match no rule and keep the framework default `public, max-age=0` (revalidated) — old bookmarks, debug requests and stale code never get a year-long lifetime. A 404 under a valid-shaped token carries `private, no-cache, no-store`. `static-scene.json` untouched. All verified on the production server with curl (tokens read from the served index). |
| Runtime | No behavioural change: `FetchChunkDataSource`, orchestrator, adapter, portal controller and `AweSpatialRuntime.init` pass the whole URL to `fetch`; nothing parses, strips or rebuilds a token (stubbed-fetch + source-scan tests). The token never appears in a navigation URL (`?destination=<id>&from=spatial` only). |
| Gate / cannabis | Hub → Cannabis → `gateRequired` → **zero** requests for the versioned cannabis chunk URL (browser-verified; 0 cannabis requests across the run). The versioned cannabis URL is served immutable like every artifact — delivery metadata, not authorisation, not prefetch; D-006 unchanged. Static gated-file availability remains the known non-M0 limitation and is **not** solved by this step. |
| Browser validation | Fresh headless-Chrome profile (GPU-backed, raw CDP) against `next build` + `next start --port 3200`: first Music visit network `200` 1 394 B (`chunk-fetch` 22.6 ms); every revisit of a versioned URL `fromDiskCache: true`, 0 wire bytes, `transferSize 0`, `deliveryType "cache"`, **no `If-None-Match`** (`chunk-fetch` 2.2–4.3 ms, travel 8–14 ms); under 150 ms latency a cache-cold chunk 165 ms vs its revisit 19 ms; reload revalidates only the index (`304`, 259 B) and serves global scene + Hub from disk. No console error. |
| Checks | 235 app vitest tests (was 213; +9 content-versioning, +7 URL-consumer, +6 cache-policy, 4 exact-URL assertions tightened, none weakened), 56 manifest tests, `check` (incl. strict `checkJs` pass), `spatial:validate/generate/check` (second generation writes nothing), `generate:check`, `next build` pass. |
| Ghost boundary | No Ghost code read, copied, cherry-picked or depended on; engine/engine-edit/studio untouched. |
...
| Gated chunk payloads are statically served | Open (known non-M0 limitation) — `public/data/spatial/chunks/cannabis-21.json` is a static file; "never fetched before the gate" is an implemented and tested application-layer rule (Step 2B.2), not a hosting-layer one, until a server-side enforcement design exists. Unchanged by 2B.4B: the versioned cannabis URL is served `immutable` like every artifact, which is delivery policy and neither authorises nor prefetches it (browser-verified: 0 cannabis requests) |
...
### M0 implementation direction (D-016)  [IN PROGRESS — application-layer boundary; chunk orchestration VERIFIED in Step 2B.2, physical portals VERIFIED in Step 2B.3, lifecycle/shutdown hardening VERIFIED in Step 2B.4A, versioned immutable delivery VERIFIED in Step 2B.4B; mobile (2B.4C) PLANNED]

For M0, chunk orchestration and destination travel are implemented in the NRVNAVerse application layer (`apps/the-nrvnaverse`) over official AWE runtime APIs, with stable destination IDs resolving to physical chunk/spawn data and visitor gates evaluated before gated fetches (D-016). Step 2B.2 delivered the one-active-chunk orchestrator on `ComponentManager.create/destroy`; Step 2B.3 delivered physical portal sensors (official mesh + `isSensor` collider + `Component3D.onSensorEnter`) that route a stable destination id into the same travel path; Step 2B.4A hardened the runtime lifecycle (asynchronous orchestrator/runtime shutdown, boot/dispose coordination with an explicit run state, boot-failure recovery, stale-portal-microtask protection) and closed the carried disposal risk; Step 2B.4B measured the loading path and delivered the performance step as HTTP policy only — content-versioned, `immutable` global-scene/chunk URLs behind a revalidated spatial-index version root, with application cache and prefetch rejected for M0 on the measurements; mobile / budgets (2B.4C) remain. This is an M0 implementation boundary, not a claim that chunk streaming is complete, and not an adoption of Ghost's experimental chunk/portal code — that work remains EXPERIMENTAL per Section 13 and D-013. Generic primitives may later move to `contrib/*` or upstream once validated.
