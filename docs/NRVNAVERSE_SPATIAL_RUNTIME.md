# NRVNAVerse Spatial Runtime — M0 Step 2A + 2B.2 + 2B.3 + 2B.4A + 2B.4B + 2B.4C
...
M0 is **not** declared complete here: 2B.4C is implemented on `feat/m0-mobile-guardrails` and awaits independent review and integration; the Landmark stays at v0.1. No new architectural decision was needed — D-004, D-006, D-009, D-013 and D-016 govern this work.
...
### 16.2 Touch interaction path (unchanged code, VERIFIED emulated)

`touchstart/move/end/cancel` on the canvas → `BrowserInputCapture` (`packages/engine/src/input/input-capture.ts:83-86`, target `CANVAS`) → `TouchState` primary-touch delta (`control-state.ts:538-568`) → `Touch.delta()` binding (`bindings.ts:980-1060`, no pointer-lock gate) → app `Look` input (`awe-spatial-runtime.ts:69`, scaled −1) → `cameraRig.rotate()` every fixed update (`:447`) → `ThirdPersonCameraRig.rotate` → `applyAxisDampening` (`base-camera-rig.ts:110-122`, `IS_TOUCH` factors). The joystick and jump button write `sharedControlState.touch.setJoystick` / `custom.pressButton("jump")` from their own DOM elements (siblings of the canvas, pointer capture), so joystick touches never reach the canvas listener and never become look.
...
Caveats from source (not measurable in emulation): a desktop-UA touch device (Windows touch laptop) gets a weaker, axis-inverted touch-look, no touch `preventDefault` from the Mediator (`mediator.ts:322,353` are `IS_TOUCH`-gated) and a pointer-lock request on tap (`mediator.ts:317-318`, `base-camera-rig.ts:243-264`) — mitigated at the app level by `touch-action: none` / `overscroll-behavior: none` (§16.4), not fixed in the engine.
...
Real-device validation (iOS Safari and Android Chrome: safe areas, pull-to-refresh, double-tap zoom on HUD text, software keyboard, thermal / fps) — **REQUIRED LATER**, not claimed here; adaptive quality with representative art; the desktop-UA touch-device caveats (§16.2) if such devices matter; visual design of the HUD (a product decision, Governance rule 9).
