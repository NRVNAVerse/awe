...
The spatial config may name an asset registry (`"assetRegistry": "asset-registry.json"`, optional and additive like `portals`). A scene component then names a **logical asset** (`assetRef: "ast_…"`) instead of a binary URL; validation runs the NRVNAVerse rights / provenance gate, and generation replaces `assetRef` with the current revision’s content-addressed runtime URL (`/assets/art/<assetId>.<first 32 hex of SHA-256>.glb`) in every generated artifact — so a new asset revision is a new chunk digest, while renaming an asset changes nothing. `generate` / `check` verify the stored binary byte-for-byte against the registry. The 64 KiB / 64-component warnings of §16 still measure spatial JSON only; asset bytes are measured by the asset gate’s M1 experimental warning bands and by `pnpm browser:perf`. Full contract: [`NRVNAVERSE_ASSET_PIPELINE.md`](./NRVNAVERSE_ASSET_PIPELINE.md).
